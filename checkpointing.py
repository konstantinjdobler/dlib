...
def my_modded_dist_load(
    ...
    *,
    ...
    # with _profile():
    storage_reader = typing.cast(StorageReader, _storage_setup(storage_reader, checkpoint_id, reader=True))
...
def dlib_save_checkpoint_hf(
    ...
    ### Save states to disk ###
    ###########################
    # Save all keys seperately to reduce peak memory usage
    ...
    with context:
        ...
        def clean_keys(save_payload):
            # don't use dict comprehension here, as it will first create a copy of the dict, causing peak memory usage to double
...
def load_optimizer_checkpoint(
    ...
    if isinstance(_unwrap_objects(model), DistributedDataParallel):
        ...
        optim_sd = get_optimizer_state_dict(
            unwrapped_model, unwrapped_optim, options=StateDictOptions(cpu_offload=False, full_state_dict=True)
        )
        # optim_sd = optimizer.state_dict()
        print("prev", optim_sd["state"].keys())
        print("prev2", optim_sd)
        ...
        assert len(list(optim_sd["state"].keys())) > 0
        assert len(list(optim_sd["state"])) == len([n for n, p in model.named_parameters()])

        printer.info(f"Time to torch load optimizer: {time.perf_counter() - t0:.02f} seconds.", rank0_only=False)
        t1 = time.perf_counter()
        optimizer.load_state_dict(optim_sd)
        printer.info(f"Time to scatter optimizer state dict: {time.perf_counter() - t1:.02f} seconds.", rank0_only=False)
        fabric.barrier()
        return
    ...
    if checkpoint_type == StateDictType.FULL_STATE_DICT:
        ...
        unwrapped_model, unwrapped_optim = _unwrap_objects([model, optimizer])
        # NOTE: SUPER IMPORTANT to use cpu_offload=True, otherwise we take ~>10GB more CUDA RAM during training (which breaks training if optimized for thorughput previously)
        optim_sd = get_optimizer_state_dict(

...
def main():
    ...
    print(f"Model trained and saved to {args.model_name}.model")
    
    print("Converting to HuggingFace format...")
    sp_file = f"{args.model_name}.model"
    tmp_hf_dir = f"{args.model_name}_hf"
    hf_tokenizer = LlamaTokenizer(vocab_file=sp_file, pad_token="<pad>", fuse_unk=False)
    hf_tokenizer.save_pretrained(tmp_hf_dir)
    hf_tokenizer.add_special_tokens({"pad_token": "<pad>"})
    
    auto_tokenizer = AutoTokenizer.from_pretrained(tmp_hf_dir)
    auto_tokenizer.save_pretrained(args.model_name)
    # Delete the temporary directory
    import shutil
    shutil.rmtree(tmp_hf_dir)
    print(f"HF tokenizer saved to {args.model_name}")
